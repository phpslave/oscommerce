 ...
 */
  ...
  class PayPalPayflowHosted extends \osCommerce\OM\Core\Site\Shop\PaymentModuleAbstract {
    protected function initialize() {
      ...
      $this->_method_title = OSCOM::getDef('paypal_payflow_hosted_method_title');
      $this->_status = (MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_STATUS == '1') ? true : false;
      $this->_sort_order = MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_SORT_ORDER;

      if ( $this->_status === true ) {
        if ( (int)MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_ORDER_STATUS_ID > 0 ) {
          $this->order_status = MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_ORDER_STATUS_ID;
        }

        if ( (int)MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_ZONE > 0 ) {
          $check_flag = false;
          ...
          $Qcheck->bindInt(':geo_zone_id', MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_ZONE);
    ...
    public function preConfirmationCheck() {
    	...
    	$custEmail = $Qaccount->value('customers_email_address');
      	...
      	$nvpStr = 'USER=' . MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_USER
      		. '&VENDOR='.MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_VENDOR
      		. '&PARTNER='.MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_PARTNER
      		. '&PWD='.MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_PASSWORD;
	  	$nvpStr .= "&TRXTYPE=" . $trxType . "&CURRENCY=USD"
			. "&TENDER=C" //C = Credit Card
			. "&AMT=" . $OSCOM_ShoppingCart->getTotal()
			. "&VERBOSITY=HIGH" 
			. "&BILLTOFIRSTNAME=" . $OSCOM_ShoppingCart->getBillingAddress('firstname')
			. "&BILLTOLASTNAME=" . $OSCOM_ShoppingCart->getBillingAddress('lastname')
			. "&BILLTOSTREET=" . $OSCOM_ShoppingCart->getBillingAddress('street_address') 
			. "&BILLTOSTREET2=" 
			. "&BILLTOCITY=" . $OSCOM_ShoppingCart->getBillingAddress('city')
			. "&BILLTOSTATE=" . $OSCOM_ShoppingCart->getBillingAddress('zone_code')
			. "&BILLTOZIP=" . $OSCOM_ShoppingCart->getBillingAddress('postcode')
			. "&BILLTOPHONENUM=" . $OSCOM_ShoppingCart->getBillingAddress('telephone')
			. "&EMAIL=" . $custEmail
			. "&SHIPTOFIRSTNAME=" . $OSCOM_ShoppingCart->getShippingAddress('firstname')
			. "&SHIPTOLASTNAME=" . $OSCOM_ShoppingCart->getShippingAddress('lastname')
			. "&SHIPTOSTREET=" . $OSCOM_ShoppingCart->getShippingAddress('street_address')
			. "&SHIPTOSTREET2="
			. "&SHIPTOCITY=" . $OSCOM_ShoppingCart->getShippingAddress('city')
			. "&SHIPTOSTATE=" . $OSCOM_ShoppingCart->getShippingAddress('country_iso_code_2')
			. "&SHIPTOZIP=" . $OSCOM_ShoppingCart->getShippingAddress('postcode');
    ...
    private function getToken($apiStr){
    	if (MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_TRANSACTION_SERVER == 'Live') {
    ...
    public function process() {
      ...
      $response_array = $this->doPayflowHostedPayment($params);
    ...
    protected function setPayflowHosted($parameters) {
      if ( MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_TRANSACTION_SERVER == 'Live' ) {
      ...
      $params = array('VERSION' => $this->_api_version,
                      'METHOD' => 'SetPayflowHosted',
                      'PAYMENTACTION' => ((MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_TRANSACTION_METHOD == 'Sale') || (strlen(MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_API_USERNAME) < 1) ? 'Sale' : 'Authorization'),
                      'RETURNURL' => OSCOM::getLink(null, null, 'ppx=retrieve', 'SSL', true, false),
                      'CANCELURL' => OSCOM::getLink(null, null, 'ppx=cancel', 'SSL', true, false));

      if ( strlen(MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_API_USERNAME) > 0 ) {
        $params['USER'] = MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_API_USERNAME;
        $params['PWD'] = MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_API_PASSWORD;
        $params['SIGNATURE'] = MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_API_SIGNATURE;
      } else {
        $params['SUBJECT'] = MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_SELLER_ACCOUNT;
      }

      if ( MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_ACCOUNT_OPTIONAL == '1' ) {
    ...
    protected function getPayflowHostedDetails($token) {
      if ( MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_TRANSACTION_SERVER == 'Live' ) {
        $api_url = 'https://api-3t.paypal.com/nvp';
      } else {
        $api_url = 'https://api-3t.sandbox.paypal.com/nvp';
      ...
      $params = array('VERSION' => $this->_api_version,
                      'METHOD' => 'GetPayflowHostedDetails',
                      'TOKEN' => $token);
    ...
    protected function doPayflowHostedPayment($parameters) {
      if ( MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_TRANSACTION_SERVER == 'Live' ) {
        $api_url = 'https://api-3t.paypal.com/nvp';
      } else {
        $api_url = 'https://api-3t.sandbox.paypal.com/nvp';
    ...
    protected function initializePayflowHosted() {
      $OSCOM_Currencies = Registry::get('Currencies');
      $OSCOM_ShoppingCart = Registry::get('ShoppingCart');
      $OSCOM_Tax = Registry::get('Tax');
      ...
      if ( !is_null($cheapest_rate) ) {
        if ( (MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_INSTANT_UPDATE == '1') && ((MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_TRANSACTION_SERVER != 'Live') || ((MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_TRANSACTION_SERVER == 'Live') && (ENABLE_SSL == true))) ) { // Live server requires SSL to be enabled
          $params['CALLBACK'] = OSCOM::getRPCLink (null, 'Cart', 'PayPal&PayflowHostedInstantUpdate', 'SSL', false, false);
...
// don't recalculate currency values as they have already been calculated
      ...
      $response_array = $this->setPayflowHosted($params);
    ...
    protected function cancelPayflowHosted() {
      unset($_SESSION['Shop']['PM']['PAYPAL']);

      OSCOM::redirect(OSCOM::getLink(null, 'Cart', null, 'SSL'));
    }

    protected function retrievePayflowHosted() {
      $response_array = $this->getPayflowHostedDetails($_GET['token']);
    ...
    protected function sendDebugEmail() {
      if ( strlen(MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_DEBUG_EMAIL) > 0 ) {
        $email_body = '$_POST:' . "\n\n";
        ...
        $email = new Mail(null, MODULE_PAYMENT_PAYPAL_PAYFLOW_HOSTED_CHECKOUT_DEBUG_EMAIL, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, 'PayPal Express Debug E-Mail');
